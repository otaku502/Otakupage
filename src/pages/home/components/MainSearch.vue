...
function initCurrentIndex() {
	currentIndex.value =
		searchList.findIndex(search => search.enName === settingStore.settings.search) || 0;
}

watch(
	() => settingStore.settings.search,
	() => {
		initCurrentIndex();
	},
	{ immediate: true }
...
function search() {
	...
	window.open(
		`${currentSearch.value.url}?${currentSearch.value.key}=${encodeURIComponent(keyword.value)}`
	);
	clearNoticeKey();
...
function keyPrev(e: Event) {
	e.preventDefault();
	selectedIndex.value =
		(selectedIndex.value - 1 + noticeKeyList.value.length) % noticeKeyList.value.length || 0;
	keyword.value = noticeKeyList.value[selectedIndex.value];
	...
	<div my-32 flex-center>
		<div flex bg-gray-200 h-44 class="search" dark="bg-18181a" style="position: relative">
			<div
				...
				v-show="showKeyDownSel"
				v-on-click-outside="handleKeyRecomend"
				absolute
				z-9
				class="search-sel"
				style="top: 100%; width: 100%; height: 10rem"
			>
				<!-- keys recommend -->
				<div z-9 bg-fff l-0 t-100p dark="border-grey-8 bg-18181a">
					<div
						v-for="(item, i) in noticeKeyList.slice(1)"
						:key="i + 1"
						text-14
						md="text-15"
						lg="text-15"
			...
			<div
				v-on-click-outside="() => (selectionVisible = false)"
				relative
				flex-center
				w-44
				class="search-sel"

...
function handleAllCommit(e: Event) {
	if (
		modalStore.inputValues.name.length <= 0 ||
...
<template>
	...
	>
		...
		<div>
			<n-input
				:status="
					errorInput && modalStore.inputValues.name.length <= 0 ? 'error' : 'success'
				"
		...
		<template #action>
			<div flex gap-x-12>
				...
				<n-button
					v-if="modalStore.action === 'update'"
					type="error"
					@click="modalStore.handleDelete"
				>
					删除
				</n-button>

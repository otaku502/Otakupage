...
export const useModalStore = defineStore('modal', () => {
	...
	function getCommonProps<T extends object, U extends object>(
		obj1: T,
		obj2: U
	): CommonProperties<T, U> {
		const commonProps = {} as CommonProperties<T, U>;
		Object.keys(obj1).forEach(key => {
			const K = key as keyof CommonProperties<T, U>;
			if (obj1[K] !== undefined && obj2[K] !== undefined) commonProps[K] = obj2[K];
		});
		return commonProps;
	}
	function showModal(
		...
		if (actionType === 'update') {
			const updateTarget = {
				site: () =>
					getCommonProps(
						inputValues,
						siteStore.data[siteStore.cateIndex].groupList[groupIndex].siteList[
							siteIndex
						]
					),
				group: () =>
					getCommonProps(
						inputValues,
						siteStore.data[siteStore.cateIndex].groupList[groupIndex]
					),
